...
\subsection{$B^+$ Tree}
A $B^+$ tree is able to execute range queries very efficiently because the leaves are ordered and linked. Also the \emph{neighbor}$(v,T)$ operation described in Section \ref{sec:Op} can be efficiently performed by a $B^+$ tree. 

\subsubsection{The Structure of $B^+$ trees}
A $B^+$ tree is organized in bocks, as implied by its name. All paths from the root to a leaf have the same length. There are three types of nodes that may exist in a $B^+$ tree: the root, interior nodes and leaves as you can see in Figure \ref{fig:cat}. The parameter \emph{n} determines the size of the blocks in the $B^+$ tree. Hence, the blocks can have maximum \emph{n} search-keys and \emph{n} + 1 pointers, pointing to its child nodes. 
\\The $B^+$ tree used for the implementation of the streaming time series data uses the temperature data as search-keys. Especially because the \emph{neighbor}$(v,T)$ operation needs the leaves to be sorted by the temperature. But since the temperature is not always unique in the time window $W$, the $B^+$ tree must be able to handle duplicate values. Other properties used, that are used in the traditional $B^+$ tree as well, are the following: \\
\begin{itemize}  
	\item 
	\item 
\end{itemize}
%TODO: difference to B trees
