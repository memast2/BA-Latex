...
But the circular array presented in the present thesis is slightly different. The value must not be associated by a pointer, but can also be directly stored in the circular array. Besides, instead of an empty field to identify the next update position, the position is stored in a variable and updated with every insertion. 
\begin{figure}[ht]
	\centering
	\includegraphics[width=1\textwidth]{ca.jpg}
	\caption{$B^+$ tree presented in \cite{BScT}.}
