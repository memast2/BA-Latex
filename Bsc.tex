...
\begin{abstract}
A streaming time series is an unbounded sequence of data points that is continuously extended, potentially forever. Many applications need to be fed continuously with the latest data. The data points arrive in a predefined interval, e.g. every 5 minutes a new value arrives. Such time series are relevant to applications in diverse domains. Imagine a meteorology station that sends a temperature measurement every 3 minutes or imagine a trader in the financial stock market who depends on updated pricing information.\\
We present the implementation of an index structure for streaming time series data. Our system keeps a limited size of time series data in main memory. As a result, it is able to access the information of past measurements in time window $W$. We introduce an implementation approach using two data structures, a circular array and a $B^+$tree, to efficiently access the data of past measurements .
...
\begin{algorithm}[H]
	...
	\caption{new\_neighborhood$(t,v,j,l)$}	\label{initializeNeighborhood}
\end{algorithm}
...
\begin{algorithm}[H]
	...
	\SetAlgoLined
	\DontPrintSemicolon
	\KwData{Query pattern value $q_{ij}$ and the set of visited time points $T$}
	\KwResult{Time point of the next most similar value to $q_{ij}$}
...
\begin{mydef}
	The neighbor$(v,T)$ takes $O(|T|)$ time.
\end{mydef}
\begin{proof}
...
\begin{mydef}
	The space complexity of all circular, doubly linked lists in the $B^+$tree is $O(|W|)$.
\end{mydef}
\begin{proof}
	For every measurement in the circular array of size $|W|$ a time point is stored in a linked list in the $B^+$tree.
\end{proof}
...
\end{mydef}
\begin{proof}
	For every measurement in the circular array space for the measurement value and the time point is allocated in the tree. 
\end{proof}

\begin{mydef}
	The space complexity of all neighborhood in a time series $s$ is $O(l \times (|W|)$. 
\end{mydef}
\begin{proof}
