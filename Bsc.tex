...
Using the sorted access mode, the algorithm finds the next yet unseen time point $t_s \notin T$ for which the value is most similar to a given measurement in a pattern cell $P_{ij}$. The time point $t_s \notin T$ is added to $T$. The time point $t_s$ has a corresponding pattern $P_{t_s}$ which is at least for one pattern cell similar to the query pattern cell $P_{ij}$.\\
...
The circular array stores the data, containing all measurement time stamps and values, the size, the last update position and a counter, which counts the total number of measurements in the  array. 
...
\subsection{Observation}
The $B^+$tree has properties that allow an efficient implementation of the $neighbor(v,t)$ and the 
...
\begin{algorithm}[H]
	...
	\SetAlgoLined
	\DontPrintSemicolon
	\KwData{Insertion node $node$ and pointer to the linke list $L$, the time point to delete $t$}
...
\begin{algorithm}[H]
	...
	\Begin{
		...
		keyPositionIndex $\leftarrow$ right position index in leaf\;
					\BlankLine
		\eIf{list on pointer at keyPositionIndex has multiple list values}{
						\BlankLine
